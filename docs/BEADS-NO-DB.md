...
**What matters:**
- `config.yaml` — your settings (track in git)
- `backup/*.jsonl` — your portable data (track in git)
- `metadata.json` — backend identity, regenerated by `bd init` (gitignored, but do NOT corrupt it while Dolt is running — see Troubleshooting)
